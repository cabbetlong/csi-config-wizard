...
- **两级防错**：表单实时校验（配置 DSL 驱动，下一步/结果页守卫拦截）+ 跨文件一致性自动保持（模板引用全局状态，无需手工核对）
- **页面与配置完全分离**：所有字段/规则/家族矩阵/平台预设/双语文案均为 `public/config/` 下的 YAML 数据，加配置无需改代码、无需重新构建（运行时加载，刷新即生效）
